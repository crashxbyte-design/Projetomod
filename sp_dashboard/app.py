...
class MainApp(QMainWindow):
    def __init__(self, data):
        ...
        # ── Ícone da Janela ─────────────────────────────────────────────────
        self.setWindowIcon(self.create_mackenzie_icon())

        # ── Cor da Titlebar (Windows 11) ───────────────────────────────────
        if sys.platform == "win32":
            try:
                import ctypes
                hwnd = int(self.winId())
                # DWMWA_CAPTION_COLOR = 35
                # Cor: B91C1C -> R:185, G:28, B:28
                color = ctypes.c_int(185 | (28 << 8) | (28 << 16))
                ctypes.windll.dwmapi.DwmSetWindowAttribute(
                    hwnd, 35, ctypes.byref(color), ctypes.sizeof(color))
                # DWMWA_TEXT_COLOR = 36 (Branco)
                text_color = ctypes.c_int(255 | (255 << 8) | (255 << 16))
                ctypes.windll.dwmapi.DwmSetWindowAttribute(
        ...
        self.header.set_meta(self.data.get("stats", {}))  # popula com dados reais do banco
    ...
    def create_mackenzie_icon(self):
        px = QPixmap(64, 64)
        px.fill(Qt.GlobalColor.transparent)
        painter = QPainter(px)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        
        # Fundo vermelho Mackenzie
        painter.setBrush(QColor("#B91C1C"))
        painter.setPen(Qt.PenStyle.NoPen)
        # Borda levemente arredondada para o ícone
    ...
    def reload_data(self):
        try:
            ...
            self.header.set_meta(self.data.get("stats", {}))  # atualiza metadados no cabeçalho
            self.build_pages()
