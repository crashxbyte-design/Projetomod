...
class BaseDadosPanel(QWidget):
    ...
    def _build_tab_subindicadores(self):
        ...
        self.tbl_sub = QTableWidget(0, 6)
        self.tbl_sub.setHorizontalHeaderLabels(["ID","Indicador Pai","Nome do Subindicador","Ordem","Ativo","Modo"])
        ...
        # Campo modo de lançamento
        sep_modo = QFrame(); sep_modo.setFrameShape(QFrame.Shape.HLine)
        sep_modo.setStyleSheet("background:#E2E8F0;")
        card_ly.addWidget(sep_modo)
        card_ly.addWidget(_section("Modo de Lançamento"))

        self.f_sub_modo = _cbo()
        self.f_sub_modo.addItems(["Mensal", "Por Horário"])
        card_ly.addWidget(_row("Como os dados serão registrados", self.f_sub_modo))

        # Dica contextual — aparece apenas no modo Por Horário
        self._hint_horario = QFrame()
        self._hint_horario.setStyleSheet(
            "QFrame{background:#FFF7ED;border:1px solid #FED7AA;"
            "border-radius:8px;}"
        )
        hint_ly = QHBoxLayout(self._hint_horario)
        hint_ly.setContentsMargins(14, 10, 14, 10)
        hint_ly.setSpacing(10)
        hint_icon = QLabel("⏱")
        hint_icon.setStyleSheet("font-size:14pt;background:transparent;border:none;")
        hint_txt = QLabel(
            "Os valores serão lançados por horário pontual (HH:MM)\n"
            "ou período (Manhã / Tarde / Noite) na tela de Histórico."
        )
        hint_txt.setFont(QFont("Segoe UI", 9))
        hint_txt.setStyleSheet("color:#92400E;background:transparent;border:none;")
        hint_ly.addWidget(hint_icon)
        hint_ly.addWidget(hint_txt, 1)
        self._hint_horario.setVisible(False)
        card_ly.addWidget(self._hint_horario)

        self.f_sub_modo.currentTextChanged.connect(self._on_modo_changed)
    ...
    def _on_modo_changed(self, text: str):
        """Exibe/oculta a dica contextual quando o modo muda."""
        self._hint_horario.setVisible(text == "Por Horário")
    ...
    def _load_subindicadores_table(self):
        ...
        for r, s in enumerate(subs):
            ...
            # Badge de modo de lançamento
            modo = s.get("modo_lancamento", "mensal")
            if modo == "por_horario":
                it_modo = QTableWidgetItem("⏱  Por Horário")
                it_modo.setForeground(QColor("#B45309"))  # âmbar escuro
            else:
                it_modo = QTableWidgetItem("≈  Mensal")
                it_modo.setForeground(QColor("#475569"))  # cinza neutro
            it_modo.setFont(QFont("Segoe UI", 9, QFont.Weight.Medium))
            self.tbl_sub.setItem(r, 5, it_modo)
    ...
    def _on_sub_select(self, selected, _):
        ...
        # Carrega modo de lançamento
        modo_db = s.get("modo_lancamento", "mensal")
        modo_txt = "Por Horário" if modo_db == "por_horario" else "Mensal"
        self.f_sub_modo.setCurrentText(modo_txt)
    ...
    def _new_sub(self):
        ...
        self.f_sub_ativo.setChecked(True)
        self.f_sub_modo.setCurrentText("Mensal")  # padrão seguro
    ...
    def _save_sub(self):
        ...
        # Mapeia label UI -> valor no banco
        modo_txt = self.f_sub_modo.currentText()
        modo_db  = "por_horario" if modo_txt == "Por Horário" else "mensal"
