...
class SnapshotWorker(QObject):
    ...
    def __init__(self, game_dir: Path, db_path: Path,
                 vanilla_dir: Path | None = None) -> None:
        super().__init__()
        self._game_dir = game_dir
        self._db_path = db_path  # Store path, create connection on worker thread
        self._vanilla_dir_to_clear = vanilla_dir  # cleared in run() — avoids main-thread freeze

    def run(self) -> None:
        try:
            # Limpa backups vanilla obsoletos ANTES de escanear.
            # Feito aqui (thread de background) para não travar a GUI.
            if self._vanilla_dir_to_clear and self._vanilla_dir_to_clear.exists():
                import shutil as _shutil
                _shutil.rmtree(self._vanilla_dir_to_clear, ignore_errors=True)
                self._vanilla_dir_to_clear.mkdir(parents=True, exist_ok=True)
    ...
    def _check_pre_snapshot(self) -> list[str]:
        ...
        # 3. Check if vanilla backup dir exists (means mods were applied before).
        ...
        # Use the configured vanilla_dir (CDModsElite/vanilla) rather than
        # hardcoding game_dir/"CDMods"/"vanilla".
        import shutil as _shutil
        vanilla_dir = self._vanilla_dir_to_clear or (self._game_dir.parent / "CDModsElite" / "vanilla")
        if not vanilla_dir.exists():
            # Fallback: check legacy CDMods/vanilla location (migration safety)
            legacy = self._game_dir / "CDMods" / "vanilla"
            if legacy.exists():
                vanilla_dir = legacy
