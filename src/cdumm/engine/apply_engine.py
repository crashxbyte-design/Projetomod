...
class ApplyWorker(QObject):
    ...
    def __init__(self, game_dir: Path, vanilla_dir: Path, db_path: Path,
        ...
        self._cdmods_dir = vanilla_dir.parent  # CDModsElite/ — never hardcode "CDMods"
    ...
    def run(self) -> None:
        try:
            ...
            self.error_occurred.emit(f"Falha ao aplicar: {e}")
    ...
    def _apply(self) -> None:
        ...
        fp_path = self._cdmods_dir / ".apply_fingerprint"
        try:
            if fp_path.exists():
                stored = fp_path.read_text(encoding="utf-8").strip()
                if stored == fingerprint:
                    # Verify overlay PAZ is intact
                    cache_json = self._cdmods_dir / ".overlay_cache.json"
                    if cache_json.exists():
                        ...
                        if overlay_paz and overlay_paz.exists():
                            ...
                            self.progress_updated.emit(100, "Já está atualizado!")
                            self.finished.emit()
                            return
        ...
        if not file_deltas and not revert_files:
            self.error_occurred.emit("Nenhuma alteração de mod para aplicar ou reverter.")
            return
        ...
        self._cached_overlay = _load_overlay_cache(self._game_dir, cdmods_dir=self._cdmods_dir)
        ...
        self.progress_updated.emit(0, f"Aplicando {total_files} arquivo(s)...")
        ...
        if needs_backup:
            self.progress_updated.emit(2, "Fazendo backup dos arquivos originais...")
            ...
            if unbacked:
                ...
                self.error_occurred.emit(
                    f"Não é possível aplicar — estes arquivos do jogo não correspondem ao original "
                    f"e não podem ser salvos com segurança:\n\n{files_str}\n\n"
                    f"Para corrigir: Verifique os arquivos do jogo via Steam, "
                    f"depois clique em Corrigir Tudo."
                )
                return
        ...
        try:
            ...
            for file_path, deltas in file_deltas.items():
                ...
                self.progress_updated.emit(pct, f"Processando {file_path}...")
                file_idx += 1
                _yield_gil()
            ...
            if json_mods:
                ...
                for jm_idx, (mod_id, mod_name, json_source, disabled_raw) in enumerate(json_mods):
                    ...
                    self.progress_updated.emit(
                        pct, f"Aplicando patch de {mod_name} do original...")
                    try:
                ...
                if len(self._overlay_entries) == overlay_count_before:
                    msg = (f"{len(json_mods)} mod(s) JSON estavam ativos mas "
                           "não produziram alterações no jogo. Verifique o log "
                           "e clique em Configurações → Corrigir Tudo "
                           "para reconstruir os backups originais.")
            ...
            xml_rows = self._db.connection.execute(
                ...
                "ORDER BY m.priority DESC, d.id DESC"
            ).fetchall()
            if xml_rows:
                self.progress_updated.emit(58, f"Aplicando patches XML ({len(xml_rows)})...")
            ...
            if self._overlay_entries:
                ...
                def _overlay_progress(idx, total, entry_name=""):
                    ...
                    if now - _last_report[0] > 0.5 or idx == total - 1:
                        ...
                        self.progress_updated.emit(pct, f"Construindo overlay ({idx + 1}/{total})...")
                ...
                self.progress_updated.emit(86, f"Preparando overlay PAZ ({len(paz_bytes) // 1048576} MB)...")
                txn.stage_file(f"{overlay_dir}/0.paz", paz_bytes)
                self.progress_updated.emit(88, "Preparando overlay PAMT...")
            ...
            for pamt_path in sorted(pamt_paths):
                ...
                self.progress_updated.emit(pct, f"Processando {pamt_path}...")
                file_idx += 1
            ...
            for file_path in revert_files:
                pct = int((file_idx / total_files) * 80)
                self.progress_updated.emit(pct, f"Revertendo {file_path}...")
                file_idx += 1
                _yield_gil()
            ...
            # ── Phase 4: PAPGT ─────────────────────────────────────────
            self.progress_updated.emit(90, "Atualizando PAPGT...")
            ...
            self.progress_updated.emit(95, "Confirmando alterações...")
            txn.commit()
            ...
            self.progress_updated.emit(100, "Aplicação concluída!")
            self.finished.emit()
    ...
    def _merge_json_patch_deltas(
        ...
        pamt_dir = file_path.split("/")[0]
        vanilla_dir = self._cdmods_dir / "vanilla"
...
class RevertWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.error_occurred.emit(f"Falha ao reverter: {e}")
    ...
    def _revert(self) -> None:
        ...
        if not mod_files:
            self.error_occurred.emit("Nenhum dado de mod encontrado. Nada a reverter.")
            return

        total = len(mod_files)
        self.progress_updated.emit(0, f"Revertendo {total} arquivo(s) para o original...")
        ...
        try:
            for i, file_path in enumerate(mod_files):
                pct = int((i / total) * 90)
                self.progress_updated.emit(pct, f"Restaurando {file_path}...")
                _yield_gil()
            ...
            self.progress_updated.emit(91, "Removendo diretórios órfãos...")
            for d in sorted(self._game_dir.iterdir()):
                if not d.is_dir() or not d.name.isdigit() or len(d.name) != 4:
                    continue
                if int(d.name) < 36:
            ...
            self.progress_updated.emit(92, "Restaurando PAPGT...")
            ...
            self.progress_updated.emit(95, "Confirmando reversão...")
            txn.commit()
            ...
            self.progress_updated.emit(100, "Reversão concluída!")
            self.finished.emit()
