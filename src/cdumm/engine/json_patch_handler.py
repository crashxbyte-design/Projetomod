...
from cdumm.engine.paths import get_vanilla_dir as _get_vanilla_dir, get_cdmods_dir as _get_cdmods_dir
...
def convert_json_patch_to_paz(patch_data: dict, game_dir: Path, work_dir: Path) -> Path | None:
    ...
    vanilla_dir = _get_vanilla_dir(game_dir)
...
def _get_pamt_index(game_dir: Path) -> dict[str, PazEntry]:
    ...
    # Cache lives alongside the CDModsElite/ folder, not in a hardcoded "CDMods"
    # When called with vanilla_dir as game_dir, go up two levels to CDModsElite/
    if game_dir.name == "vanilla":
        cdmods = game_dir.parent  # CDModsElite/
    elif (game_dir / "CDModsElite").exists():
        cdmods = game_dir / "CDModsElite"
    elif (game_dir / "CDMods").exists():
        cdmods = game_dir / "CDMods"  # legacy fallback — read-only
    else:
        cdmods = game_dir  # last resort, store next to PAMTs
