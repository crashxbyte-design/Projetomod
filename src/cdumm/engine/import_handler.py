...
from cdumm.engine.paths import get_vanilla_dir as _get_vanilla_dir
...
def _try_paz_entry_import(
    ...
    _vdir = _get_vanilla_dir(game_dir)
    vanilla_pamt = _vdir / dir_name / "0.pamt"
    ...
    try:
        van_entries = parse_pamt(str(vanilla_pamt), paz_dir=str(
            (_vdir / dir_name)
            if (_vdir / dir_name / "0.pamt").exists()
...
def _detect_standalone_mod(
    ...
    for d in extracted_dir.rglob("*"):
        ...
        vanilla_backup_dir = _get_vanilla_dir(game_dir)
...
def import_from_bsdiff(
    ...
    vanilla_file = _get_vanilla_dir(game_dir) / target_path.replace("/", "\\")
...
def import_from_json_patch(
    ...
    # mods_dir lives inside CDModsElite/ (derived from deltas_dir) — never game_dir/"CDMods"
...
def _process_extracted_files(
    ...
    for match_idx, (rel_path, extracted_path, is_new) in enumerate(matches):
        ...
        try:
            ...
            vanilla_backup = _get_vanilla_dir(game_dir) / rel_path.replace("/", "\\")
