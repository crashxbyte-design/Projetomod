...
def _build_full_path_map(pamt_dir: str, game_dir, cdmods_dir=None) -> dict[str, str]:
    ...
    _cdmods = Path(cdmods_dir) if cdmods_dir else game_dir / "CDMods"
    result = {}

    for base in [_cdmods / "vanilla", game_dir]:
...
def _load_overlay_cache(game_dir, cdmods_dir=None) -> dict:
    ...
    import json
    from pathlib import Path
    _cdmods = Path(cdmods_dir) if cdmods_dir else Path(game_dir) / "CDMods"
    cache_path = _cdmods / ".overlay_cache.json"
...
def _save_overlay_cache(game_dir, overlay_dir, cache_entries: dict, cdmods_dir=None):
    """Save overlay cache manifest for incremental rebuild."""
    import json
    from pathlib import Path
    _cdmods = Path(cdmods_dir) if cdmods_dir else Path(game_dir) / "CDMods"
    cache_path = _cdmods / ".overlay_cache.json"
    _cdmods.mkdir(parents=True, exist_ok=True)
...
def _get_vanilla_pabgh(pamt_dir: str, pabgb_entry_path: str, game_dir, cdmods_dir=None) -> bytes | None:
