...
class VerifyWorker(QObject):
    ...
    def run(self):
        try:
            ...
            if not snap_entries:
                self.error_occurred.emit("Snapshot não rastreado. Verifique os arquivos primários do jogo primeiro.")
                db.close()
                return
            ...
            for i, (file_path, snap_hash, snap_size) in enumerate(snap_entries):
                ...
                self.progress_updated.emit(pct, f"Verificando integridade física: {file_path}...")
            ...
            db.close()
            self.progress_updated.emit(100, "Varredura Concluída!")
...
class VerifyDialog(QDialog):
    ...
    def __init__(self, results: dict, parent=None):
        ...
        self.setWindowTitle("Checagem Global do Estado de Integridade")
        ...
        header = QLabel(f'Estado Global do Jogo: <span style="color:{color};">{status}</span>')
