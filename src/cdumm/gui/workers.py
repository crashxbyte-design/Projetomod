...
class ImportWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.progress_updated.emit(0, f"Formato detectado: {fmt}")
            ...
            if fmt == "zip":
                ...
                self.progress_updated.emit(10, "Executando script...")
...
class PreHashWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            total = len(all_files)
            self.progress_updated.emit(0, f"Verificando {total} arquivos do jogo...")
            ...
            for i, rel_path in enumerate(all_files):
                ...
                if (i + 1) % 5 == 0 or (i + 1) == total:
                    ...
                    self.progress_updated.emit(pct, f"Verificados {i + 1}/{total} arquivos...")
        ...
        try:
            ...
            for i, rel_path in enumerate(self._targeted):
                ...
                if rel_path in snap_hashes:
                    ...
                    if current_hash == snap_hashes[rel_path]:
                        ...
                        if not vanilla_file.exists():
                            self.progress_updated.emit(pct, f"Fazendo backup de {rel_path}...")
                ...
                if vanilla_file.exists():
                    self.progress_updated.emit(pct, f"Restaurando {rel_path}...")
            ...
            for i, rel_path in enumerate(all_files):
                if (i + 1) % 20 == 0:
                    ...
                    self.progress_updated.emit(pct, f"Pré-verificando {i+1}/{hash_total}...")
            ...
            self.progress_updated.emit(100, "Pronto!")
            self.finished.emit(pre_hashes)
...
class ScriptCaptureWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.progress_updated.emit(0, f"Detectando alterações...")
            ...
            for file_idx, (rel_path, old_hash) in enumerate(self._pre_hashes.items()):
                if file_idx % 20 == 0:
                    ...
                    self.progress_updated.emit(
                        pct, f"Verificando {file_idx + 1}/{total_files}...")
                    time.sleep(0)  # yield GIL
            ...
            if not changed:
                ...
                result.error = (
                    "Nenhuma alteração detectada. Este mod pode já estar aplicado.\n\n"
                    "To install it fresh:\n"
            ...
            self.progress_updated.emit(20, f"Encontrado(s) {len(changed)} arquivo(s) modificado(s)...")
            ...
            if paz_files:
                self.progress_updated.emit(25, "Analisando entradas PAZ...")
    ...
    def _capture_paz_entries(
        ...
        for dir_idx, (pamt_dir, paz_rels) in enumerate(sorted(by_dir.items())):
            ...
            self.progress_updated.emit(pct, f"Analisando entradas em {pamt_dir}/...")
            ...
            for entry_idx, (path_lower, mod_entry) in enumerate(target_entries):
                ...
                if total_entries > 0 and (entry_idx % 20 == 0 or entry_idx == total_entries - 1):
                    ...
                    self.progress_updated.emit(
                        pct, f"Comparando {pamt_dir}/ entrada {entry_idx + 1}/{total_entries}...")
...
class ScanChangesWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.progress_updated.emit(0, f"Escaneando {total} arquivos do jogo...")
            ...
            for i, (rel_path, stored_hash) in enumerate(snapshot_rows):
                ...
                if (i + 1) % 10 == 0 or (i + 1) == total:
                    ...
                    self.progress_updated.emit(pct, f"Escaneados {i + 1}/{total} arquivos...")
                time.sleep(0)  # yield GIL

            if not changed:
                result = ModImportResult(self._mod_name)
                result.error = "Nenhuma alteração detectada. Os arquivos do jogo correspondem ao snapshot original."
                self.finished.emit(result)
                db.close()
                return

            logger.info("Found %d changed files: %s", len(changed), changed)
            self.progress_updated.emit(55, f"Encontrado(s) {len(changed)} arquivo(s) modificado(s)...")
            ...
            if paz_files:
                self.progress_updated.emit(60, "Analisando entradas PAZ...")
    ...
    def run(self) -> None:
        try:
            ...
            for i, full in enumerate(backup_files):
                ...
                self.progress_updated.emit(pct, f"Verificando {rel}...")
                time.sleep(0)  # yield GIL
...
class ModCheckWorker(QObject):
    ...
    def __init__(self, game_dir: Path, db_path: Path, vanilla_dir: Path | None = None):
        super().__init__()
        self._game_dir = game_dir
        self._db_path = db_path
        self._vanilla_dir = vanilla_dir or (game_dir / "CDMods" / "vanilla")  # fallback legacy
    ...
    def run(self):
        try:
            ...
            self.progress_updated.emit(10, "Verificando tamanhos dos arquivos originais...")
            try:
                ...
                for mod_name, fp, expected_size in size_rows:
                    # Use CDModsElite/vanilla — derived from deltas_dir.parent/vanilla
                    # so it follows the configured path, not a hardcoded "CDMods"
                    vanilla_path = self._vanilla_dir / fp.replace("/", os.sep)
            ...
            self.progress_updated.emit(90, "Verificando arquivos delta...")
...
class MigrateWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.progress_updated.emit(5, "Revertendo para o original...")
            try:
            ...
            for i, (mod_id, mod_name, source_path, priority) in enumerate(mods):
                ...
                self.progress_updated.emit(pct, f"Reimportando {mod_name}...")
            ...
            db.close()
            self.progress_updated.emit(100, "Migração concluída!")
