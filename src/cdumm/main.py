...
APP_DATA_DIR = Path.home() / "AppData" / "Local" / "CDMod_Elite"
...
def main() -> int:
    ...
    logger.info("Starting Crimson Desert Elite BR")
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            hwnd = ctypes.windll.user32.FindWindowW(None, f"Crimson Desert Elite BR - {__version__}")
            if hwnd:
    ...
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("kindiboy.cdummelite.modmanager.3")
    except Exception:
        pass
    ...
    app.setStyle("fusion")
    app.setApplicationName("Crimson Desert Elite BR")
    ...
    if font_path and font_path.exists():
        ...
        if font_id >= 0:
            ...
            if families:
                logger.debug("Loaded font: %s", families[0])

    # Apply Elite BR global stylesheet (crimson dark theme)
    from cdumm.gui.theme import STYLESHEET
    app.setStyleSheet(STYLESHEET)

    # First-time setup is handled by SetupDialog when game_dir is None (below).
    # Welcome wizard (Fluent) removed — EliteBR uses setup_dialog.py instead.
    _wizard_lang = "en"
    _wizard_theme = "dark"
    _wizard_game_dir = None
    _first_launch = not (APP_DATA_DIR / ".wizard_done").exists()
    ...
    splash.showMessage("  Loading database...", 0x0041)  # AlignLeft | AlignBottom
    app.processEvents()
    ...
    cdmods_dir = game_path / "CDModsElite"
    ...
    splash.showMessage("  Loading game schemas...", 0x0041)
    app.processEvents()
    ...
    splash.showMessage("  Checking game state...", 0x0041)
    app.processEvents()
    ...
    if startup_context["has_snapshot"]:
        splash.showMessage("  Verifying game files...", 0x0041)
        app.processEvents()
    ...
    splash.showMessage("  Building UI...", 0x0041)
    app.processEvents()
